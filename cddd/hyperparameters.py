...
def add_arguments(parser):
    ...
    parser.add_argument('-c', '--cell_size', help='hidden layers of cell. multiple numbers for multi layer rnn', nargs='+', default=[512, 1024, 2048], type=int) # [128]
    parser.add_argument('-e', '--emb_size', help='size of bottleneck layer', default=512, type=int) # 128
    parser.add_argument('-l', '--learning_rate', default=0.0002, type=float)  # 0.0005
    parser.add_argument('-s', '--save_dir', help='path to save and log files', default='/projects/mai/users/knfn081_emma/cddd/cddd/data/default_model', type=str) # os.path.join(DEFAULT_DATA_DIR, 'default_model')
    parser.add_argument('-d', '--device', help="number of cuda visible device", default="2", type=str)  # -1 TODO
    parser.add_argument('-gmf', '--gpu_mem_frac', default=0.1, type=float) # 1.0
    parser.add_argument('-n', '--num_steps', help="number of train steps", default=100000000, type=int) # 250000
    ...
    parser.add_argument('--input_dropout', default=0.15, type=float) # 0.0
    parser.add_argument('--emb_noise', default=0.05, type=float)  # 0.0
    ...
    parser.add_argument('--rand_input_swap', default=True, type=bool) # False
    parser.add_argument('--infer_input', default="canonical", type=str) # random
